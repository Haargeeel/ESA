...
public class ShowTouchpointService {
	...
	public void deleteTouchpoint(AbstractTouchpoint tp) {
		...
		String requestUri = "http://localhost:8888/org.dieschnittstelle.jee.esa.servlets/api/touchpoints/" + tp.getId();
	...
	public AbstractTouchpoint createNewTouchpoint(AbstractTouchpoint tp) {
		...
		try {
			String requestUri = "http://localhost:8888/org.dieschnittstelle.jee.esa.servlets/api/touchpoints";
			...
			// output stream
			ByteArrayEntity bae = new ByteArrayEntity(out.toByteArray());
			...
			// log the status line
			logger.info("statusline: " + res.getStatusLine());
			...
			// read the touchpoint object from the input stream
				AbstractTouchpoint resTouchPoint = (AbstractTouchpoint) ois.readObject();
				ois.close();
			// cleanup the request
			// EntityUtils.consume(response.getEntity());
				EntityUtils.consume(res.getEntity());
				return resTouchPoint;
			} else {
